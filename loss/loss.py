...
def zero_one_loss(y, pred, average = True):
    ...
    return np.sum(np.abs(y - pred))/float(y.shape[0]) if average else np.sum(np.abs(y - pred))
...
def cross_entropy_loss(y, pred, average = True):
    ...
    return nll/float(y.shape[0]) if average else nll
...
def hinge_loss(y, pred, average = True):
    ...
    return np.sum(hl)/float(y.shape[0]) if average else np.sum(hl)
...
def squared_l2_loss(y, pred, average = True):
    ...
    return squared_norm/float(y.shape[0]) if average else squared_norm
